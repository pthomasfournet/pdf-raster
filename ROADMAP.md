...
- [ ] **Task 4 — Device page buffer + GPU blit kernel** (~600 LoC). **GPU side shipped** in commit `6ee47de`, hardened in `738ba14`/`ef67045`: `crates/gpu/kernels/blit_image.cu` (16×16-block CUDA kernel with f32 inverse-CTM nearest-neighbour sampling matching the CPU path byte-for-byte), `crates/gpu/src/blit.rs` (`InverseCtm`, `BlitBbox`, `GpuCtx::blit_image_to_buffer`, structured `BlitError`), `crates/gpu/src/cache/page_buffer.rs` (`DevicePageBuffer` zero-init RGBA8). 4 blit tests including a parity test against a CPU reference that mirrors the kernel's f32 math. **Renderer integration pending**: `ImageData::Gpu` variant, `decode_dct → ImageData::Gpu` wiring, `PageRenderer` per-page `DevicePageBuffer`, end-of-page composite. Pre-existing AA-fill `JITTER_Y` corruption (8 wrong Halton(3) values) found and fixed in `8f01c3d` while bringing up gpu-validation tests; 110/110 gpu tests now pass.
