...
- [~] **Task 3 — Vulkan backend implementation**. `VulkanBackend` ships at `crates/gpu/src/backend/vulkan/` via `ash` 0.38 + `gpu-allocator` 0.28 (pure Rust, no FFI).  Module split: `device` (instance/device/queue init, Vulkan 1.3 features incl. shaderInt8 + bufferDeviceAddress + 8-bit storage), `memory` (slab sub-allocator wrapping gpu-allocator with persistent-mapped host buffers and BDA-queried device buffers), `pipeline` (lazy SPIR-V→VkPipeline cache, per-kernel descriptor set layouts), `recorder` (per-page command buffer, timeline-semaphore page fence, single-page-in-flight invariant, compute→compute memory barriers between dispatches), `transfer` (synchronous one-shot upload/download via host-coherent staging).  All six kernels wired except `record_blit_image` which awaits inv_ctm push-const re-port.
    - **Parity tests passing (12/12):** four `composite_rgba8`, three `apply_soft_mask`, three `aa_fill` (square/triangle/degenerate), two `icc_cmyk_clut` — all match the CPU reference within ≤ 1 LSB per channel on the dev box's nouveau ICD.
    - **Spec deviations (deliberate):** descriptor-set bindings instead of BDA push-constants (kernels emit `StorageBuffer` storage class; BDA is a perf optimisation, not load-bearing for parity); synchronous `upload_async` (the dedicated transfer queue is gated on the prefetcher critical-path measurement); `vkPipelineCache` file persistence not yet wired (cold start adds ~20 ms — small).
    - **Not yet wired:** `record_blit_image` (needs `inv_ctm` push-const re-port — the existing Slang reads it from a StructuredBuffer); `record_tile_fill` parity test (no CPU reference exists; sub-task 2a's CPU twin would unblock).
    - **Cross-vendor smoke:** pending — needs AMD/Intel hardware or `VK_ICD_FILENAMES=lavapipe.json` CI run.
