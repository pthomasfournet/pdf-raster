...
//!
//! Usage: `cargo run -p rasterrocket-interp --example scan_fills -- <pdf>`
...
fn main() {
    ...
    let n = rasterrocket_interp::page_count(&doc);
    for p in 1..=n {
        let ops = rasterrocket_interp::parse_page(&doc, p).unwrap_or_else(|e| {
        ...
        let dark_fills: Vec<_> = ops
            ...
            .filter(|o| {
                matches!(o,
                    rasterrocket_interp::content::Operator::SetFillGray(g) if *g < 0.9
                )
            })
            .collect();
        let has_rect = ops
            .iter()
            .any(|o| matches!(o, rasterrocket_interp::content::Operator::Rectangle(..)));
        let has_rgb = ops.iter().any(|o| {
            matches!(
                o,
                rasterrocket_interp::content::Operator::SetFillRgb(..)
                    | rasterrocket_interp::content::Operator::SetStrokeRgb(..)
            )
        });
