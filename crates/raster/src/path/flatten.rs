...
pub struct CurveData {
    ...
    pub cx: Box<[f64; (MAX_CURVE_SPLITS as usize + 1) * 3]>,
    /// Y coordinates: same layout as `cx`.
    pub cy: Box<[f64; (MAX_CURVE_SPLITS as usize + 1) * 3]>,
    /// `cNext[p1]` = index of the next segment's first slot after `p1`.
    pub c_next: Box<[i32; MAX_CURVE_SPLITS as usize + 1]>,
}

impl CurveData {
    pub fn new() -> Box<Self> {
        Box::new(Self {
            cx: Box::new([0.0; (MAX_CURVE_SPLITS as usize + 1) * 3]),
            cy: Box::new([0.0; (MAX_CURVE_SPLITS as usize + 1) * 3]),
            c_next: Box::new([0i32; MAX_CURVE_SPLITS as usize + 1]),
        })
    }
}
...
pub fn flatten_curve(
    ...
    let i0 = 0usize;
    let i2 = MAX_CURVE_SPLITS as usize;
    ...
    while pp1 < max {
        let pp2 = data.c_next[pp1 as usize];
        let pp2u = pp2 as usize;
        let pp1u = pp1 as usize;
...
mod tests {
    ...
    fn does_not_exceed_max_curve_splits() {
        ...
        assert!(out.len() <= MAX_CURVE_SPLITS as usize);
    }
}
