...
//!
//! The function receives pixel coordinates in device space and returns an RGB
//! triple.  Colour-space conversion (from the shading's colour space to device
//! RGB) is the caller's responsibility — the function closure already delivers
//! device-space bytes.

use crate::pipe::Pattern;

/// A shading pattern driven by an arbitrary function `f(x, y) → [R, G, B]`.
///
/// The function is stored as a `Box<dyn Fn(f64, f64) -> [u8; 3]>` so it can
/// capture ICC profiles, LUTs, or any other per-page state from `pdf_bridge`.
...
mod tests {
    ...
    #[test]
    fn identity_function_horizontal_gradient() {
        let p = FunctionPattern::new(|x, _y| {
            let v = (x as u8).wrapping_mul(1);
            [v, v, v]
        });
    ...
    #[test]
    fn y_dependent_function_uses_y() {
