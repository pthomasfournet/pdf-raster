...
impl RadialPattern {
    ...
    #[must_use]
    #[expect(clippy::too_many_arguments, reason = "mirrors PDF shading dict: 2 colors + 2 circles (cx,cy,r each) + t range + 2 extend flags")]
    pub fn new(
        color0: [u8; 3],
        color1: [u8; 3],
...
mod tests {
    ...
    fn extend_clamps_outside_to_endpoints() {
        ...
        assert!(out[0] > 240, "beyond outer with extend should clamp to color1 (white)");
    }
}
