#!/usr/bin/env bash
# v0.9.0 gpu-jpeg-huffman benchmark — four-mode corpus run.
#
# Modes:
#   A    CPU only (regression baseline).
#   D    CUDA full GPU (nvjpeg + gpu-aa + gpu-icc).
#   H    CUDA parallel-Huffman (gpu-jpeg-huffman + gpu-aa + gpu-icc).
#   V    Vulkan parallel-Huffman (vulkan feature, implies gpu-jpeg-huffman).
#
# Usage:
#   scripts/bench_v090.sh [--force]
set -euo pipefail
...
MODES=(A D H V)
declare -A MODE_BIN=([A]=cpu [D]=full-gpu [H]=cuda-huffman [V]=vulkan-huffman)
declare -A MODE_BACKEND=([A]=cpu [D]=cuda [H]=cuda [V]=vulkan)
declare -A MODE_FEATURES=(
  [A]=""
  [D]="nvjpeg,gpu-aa,gpu-icc"
  [H]="gpu-jpeg-huffman,gpu-aa,gpu-icc"
  [V]="vulkan"
)
declare -A MODE_LABEL=(
  [A]="CPU-only"
  [D]="CUDA + nvJPEG"
  [H]="CUDA parallel-Huffman"
  [V]="Vulkan parallel-Huffman"
)
...
preflight() {
  ...
  if [[ ! -e /dev/nvidia0 ]]; then
    warn "/dev/nvidia0 missing; modes D/H/V (CUDA/Vulkan on NVIDIA) will be skipped"
...
aggregate() {
  ...
  python3 scripts/aggregate_v090.py "$OUT_DIR" > "$OUT_DIR/results.md"
