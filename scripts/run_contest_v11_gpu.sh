#!/usr/bin/env bash
# Phase-11 GPU-feature re-bench.  Runs E1-E4 against the same archives
# as run_contest_v11.sh (CPU baseline), plus a single-page DCT E5 event.
#
# Output mirrors the CPU run shape but lives in bench/v11-gpu/ so the
# committed bench/v11/ baseline stays untouched.
#
# Build: --features cache,gpu-aa,gpu-icc on pdf_raster.  No PGO — we
# want the GPU vs CPU delta clean of orthogonal optimisations.
# nvjpeg deliberately omitted: GPU_JPEG_THRESHOLD_PX = u32::MAX gates
# it at the call site regardless of feature flag.
...
RESULTS_DIR="$REPO_ROOT/bench/v11-gpu"
...
echo "==========================================================="
echo "Phase 11 GPU re-bench starting $(date -Iseconds)"
echo "Features: pdf_raster/cache,pdf_raster/gpu-aa,pdf_raster/gpu-icc"
echo "==========================================================="

# --- Step 1: Build with GPU features (no PGO) ---
echo
echo "==> [1/5] Building contest_v11 + pdf-raster CLI with GPU features"
cargo build --release \
    -p bench --bin contest_v11 \
    --features "pdf_raster/cache,pdf_raster/gpu-aa,pdf_raster/gpu-icc" -q
cargo build --release \
    -p pdf-raster \
    --features "cache,gpu-aa,gpu-icc" -q
...
# E5 — single DCT page, 300 DPI, render+write timing.  pdf-raster CLI
# emits its end-to-end wall on stderr already; we time the whole process
# externally with /usr/bin/time -f for portability and use mutool draw
# / pdftoppm for comparison.
echo "    E5 (DCT-heavy single page, corpus-08 p$E5_PAGE @ ${E5_DPI} DPI)"
E5_OUR_TMPL="/tmp/p11gpu_e5_ours"
E5_MU_OUT="/tmp/p11gpu_e5_mu.ppm"
E5_PP_TMPL="/tmp/p11gpu_e5_pp"

time_ms() {
    # arg: command; prints elapsed milliseconds
    local t0 t1
    t0=$(date +%s%N)
    "$@" >/dev/null 2>&1
    t1=$(date +%s%N)
...
for kind in cold warm warm warm warm; do
    ...
    OURS_MS=$(time_ms "$RASTER" "$E5_FIXTURE" "$E5_OUR_TMPL" -f $E5_PAGE -l $E5_PAGE -r $E5_DPI)
    MU_MS=$(time_ms mutool draw -q -P -N -r $E5_DPI -F ppm -o "$E5_MU_OUT" "$E5_FIXTURE" $E5_PAGE)
    # pdftoppm uses -f/-l like our CLI; output prefix + page-number.ppm
...
## E5 — single DCT page (corpus-08 p$E5_PAGE @ ${E5_DPI} DPI, end-to-end including write)
...
echo
echo "==========================================================="
echo "GPU re-bench complete $(date -Iseconds)"
echo "Results: $MD"
echo "==========================================================="
